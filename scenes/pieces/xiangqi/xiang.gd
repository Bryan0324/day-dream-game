...
var origin = "res://scenes/pieces/xiangqi/Xiang.tscn"
...
func _get_valid_moves() -> Array:
	...
	for s1 in sign:
		for s2 in sign:
			var tmp = position_grid + Vector2i(s1*4, s2*4)
			if board.check_legal(tmp):
				if !board.check_board(position_grid + Vector2i(s1*2, s2*2)):
					unlimited_moves.append(tmp)
		
	var moves = []
	for i in unlimited_moves:
		if board.check_legal(i):
			if !board.check_board(i) && i.y <= 10:
				moves.append(i)
